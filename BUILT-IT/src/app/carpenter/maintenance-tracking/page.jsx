...
export default function MaintenanceTracking() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch carpenter complaints and students data in parallel
        ...
        // Filter only carpenter complaints - this is redundant if API already filters,
        // but keeping it as a safety measure in case API changes
        const carpenterComplaints = enhancedComplaints.filter(complaint => 
          complaint.type === 'carpenter'
        );
        
        setComplaints(carpenterComplaints);
      } catch (error) {
  ...
  // Compute summary counts for bar graph - carpenter complaints only
      ...
      <div className={`min-h-screen flex items-center justify-center ${bgClass}`}>
        <div className="flex flex-col items-center">
          ...
          <p className={`mt-4 ${textColors.description}`}>
            Loading carpenter maintenance tasks...
          </p>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className={`text-2xl font-bold ${textColors.title} flex items-center`}>
              ...
              carpenter Maintenance Tracking
            </h1>
            <p className={`text-sm ${textColors.description}`}>
              carpenter complaints management system
            </p>
          </div>
          <Link href="/carpenter" className="flex items-center px-3 py-2 rounded-md transition-colors text-gray-600 hover:text-gray-800 bg-gray-100 hover:bg-gray-200">
                    ...
                    {filteredComplaints.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="px-6 py-16 text-center text-gray-500">
                          ...
                          <h3 className="text-xl font-medium text-gray-700">No carpenter complaints found</h3>
                          <p className="text-gray-500 mt-2">No carpenter complaints match the current filter.</p>
                        </td>
                      </tr>
                    ) : (
                      filteredComplaints.map((complaint) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm text-gray-700">
                            ...
                            <div className="text-xs text-gray-500 mt-1">Type: carpenter</div>
                          </td>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              <div className="bg-gradient-to-r from-indigo-50 to-indigo-100 px-6 py-4 rounded-t-xl">
                <h2 className={`text-xl font-semibold ${textColors.title}`}>carpenter Complaints Overview</h2>
              </div>
              <div className="p-6">
                <div className="h-64">
                  <Bar 
            ...
            {/* Hostel Distribution - carpenter Complaints */}
            <motion.div 
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              <div className="bg-gradient-to-r from-green-50 to-green-100 px-6 py-4 rounded-t-xl">
                <h2 className={`text-xl font-semibold ${textColors.title}`}>carpenter Complaints by Hostel</h2>
              </div>
              <div className="p-6">
        ...
        >
          ...
          >
            <div className="bg-gradient-to-r from-red-500 to-red-700 text-white px-6 py-4 rounded-t-xl">
              <h3 className="text-xl font-medium">Update carpenter Complaint #{editingComplaint.complaint_id}</h3>
            </div>
